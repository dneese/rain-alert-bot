...
// Persist per-location monitoring state (rain transition, message id, cooldown,
// advance-warning dedup) directly in user_locations columns.
export async function updateUserLocationState(chatId, locId, data) {
  await supaPatch('user_locations', `id=eq.${locId}&chat_id=eq.${chatId}`, data);
}
